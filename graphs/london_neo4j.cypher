...
CREATE (a)-[:CONNECTED_THROUGH {line: row.Tube_Line, color: row.Color, type: row.Type}]->(b)
CREATE (b)-[:CONNECTED_THROUGH {line: row.Tube_Line, color: row.Color, type: row.Type}]->(a);
