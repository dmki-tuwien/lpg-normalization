...
def test_node_pattern():
    ...
    a_dep_1 = GOFD.from_string("(a:Test_0a)::a=>a")
    a_dep_2 = GOFD.from_string("(a::p)::a=>a")
    a_dep_3 = GOFD.from_string("(a:Test_2c:q)::a.q=>a,a.q")
...
def test_concat_pattern():
    ab_dep_0 = GOFD.from_string("(a),(b)::a=>b")
    ab_dep_1 = GOFD.from_string("(a),()::a=>a")
    ab_dep_2 = GOFD.from_string("(a:Test1:p)<-[c]-(d:Hallo:t),(b:Test2:q)-[]->()::a.p=>b.q")
...
def test_edge_pattern():
    abc_dep_0 = GOFD.from_string("(a)-[b]->(c)::a=>b")
    ...
    abc_dep_1 = GOFD.from_string("(a)<-[b]-(c)::a=>b")
    ...
    abc_dep_2 = GOFD.from_string("(a::p)-[b::hi]->(c)::a=>b")
    ...
    abc_dep_3 = GOFD.from_string("(a::p)<-[b::hi]-(c)::a=>b")
    ...
    abc_dep_4 = GOFD.from_string("(a::p)<-[]-(c)::a.p=>c")
    ...
    abc_dep_5 = GOFD.from_string("(a::p)<-[:My_Label]-(c)::a.p=>c")
    ...
    abc_dep_6 = GOFD.from_string("(a::p)<-[b:My_Label]-(c)::a.p=>b")
    ...
    abc_dep_7 = GOFD.from_string("(a::p)<-[b:My_Label:Prop]-(c)::a.p=>b.Prop")
...
def test_vars_in_scope():
    with pytest.raises(ValueError):
        GOFD.from_string("(a)-[b]->(c)::a.x=>b.y")

    with pytest.raises(ValueError):
        GOFD.from_string("(a)-[b]->(c)::a=>by")
...
def test_pattern_equality():
    ...
    abc_dep_1 = GOFD.from_string("(a)<-[b]-(c:Hi)::a=>b")
    ...
    abc_dep_2 = GOFD.from_string("(a::D)<-[b]-(c)::a=>b")
    ...
    # Edges vs. concat
    abcde_dep_1 = GOFD.from_string("(a::D)<-[b]-(c)-[d]->(e:Hello:World)::a=>b")
    abcde_dep_2 = GOFD.from_string("(a::D)<-[b]-(c),(c)-[d]->(f:Hello:World)::c=>f.World")

    assert abcde_dep_1.pattern.equals(abcde_dep_2.pattern)

    abcde_dep_3 = GOFD.from_string("(a::D)<-[b]-(c)-[d]->(e:Hello:World)::a=>b")
    abcde_dep_4 = GOFD.from_string("(a::D)<-[b]-(c),(c:No_longer_equal)-[d]->(f:Hello:World)::c=>f.World")
    abcde_dep_5 = GOFD.from_string("(a::D)<-[b]-(t),(t:No_longer_equal)-[d]->(f:Hello:World)::t=>f.World")

    assert not abcde_dep_3.pattern.equals(abcde_dep_4.pattern)
    assert abcde_dep_5.pattern.equals(abcde_dep_4.pattern)

    # Nodes vs. concat
    ab_dep_0 = GOFD.from_string("(a),(b::Hi)::a=>b.Hi")
    ab_dep_1 = GOFD.from_string("(b::Hi)::b=>b.Hi")

    assert not ab_dep_0.pattern.equals(ab_dep_1.pattern)

def test_pattern_more_specific(): # Note that more specific patterns are "less than" more general patterns
    ab_dep_0 = GOFD.from_string("(a),(b::Hi)::a=>b.Hi")
    ab_dep_1 = GOFD.from_string("(b::Hi)::b=>b.Hi")

    assert ab_dep_0.pattern.less_than(ab_dep_1.pattern)

    abc_dep_1 = GOFD.from_string("(a::D)<-[b]-(c)::a=>b")
    abc_dep_2 = GOFD.from_string("(a::D&E)<-[b]-(c)::a=>b")
    abc_dep_3 = GOFD.from_string("(a::D&E)<-[b:L]-(c)::a=>b")
    abc_dep_4 = GOFD.from_string("(a:K:D&E)<-[b:L]-(c)::a=>b")


    assert abc_dep_2.pattern.less_than(abc_dep_1.pattern)
...
#     a_dep_0 = GOFD.from_string("(a)::a=>a")
#     a_dep_1 = GOFD.from_string("(a:Test_0a)::a=>a")
#     a_dep_2 = GOFD.from_string("(a:Ole:p)::a=>a")
#     a_dep_3 = GOFD.from_string("(b:Test_2c:q)::b.q=>b,b.q")
#     a_dep_23 = GOFD.from_string("(c:Test_2c&Ole:p&q)::c.q=>c,c.q")
#
